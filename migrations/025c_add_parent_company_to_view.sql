-- Migration 025c: Add parent_company to jobs_with_employer_context view
-- Part of: Epic - Job Feed UX Enhancement
-- Date: 2026-01-05
-- Purpose: Expose parent_company field for subsidiary display (e.g., "Alphabet" for YouTube)
--
-- Depends on: Migration 025b (must run first)
-- Note: parent_company column already exists in employer_metadata (added in 025)

-- =============================================================================
-- Drop and recreate view (adding new column requires DROP)
-- =============================================================================
...
SELECT
    ...
    em.parent_company AS employer_parent_company,  -- NEW: e.g., "Alphabet" for YouTube
...
-- Check view includes parent_company
SELECT column_name
FROM information_schema.columns
WHERE table_name = 'jobs_with_employer_context'
  AND column_name = 'employer_parent_company';

-- Sample query to verify
-- SELECT employer_name, employer_parent_company
-- FROM jobs_with_employer_context
-- WHERE employer_parent_company IS NOT NULL
-- LIMIT 10;
